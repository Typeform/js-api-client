...
export class TypeformResponses {
   ...
   * @param args.pageSize Maximum number of responses. Default value is 25. Maximum value is 1000.
   ...
   * @param args.since Limit request to responses submitted since the specified date and time.
   * In ISO 8601 format, UTC time, to the second, with T as a delimiter between the date and time.
   * @param args.until Limit request to responses submitted until the specified date and time.
   * In ISO 8601 format, UTC time, to the second, with T as a delimiter between the date and time.
   * @param args.after Limit request to responses submitted after the specified token. If you use the after parameter,
   ...
   * @param args.before Limit request to responses submitted before the specified token. If you use the before parameter,
   * the responses will be sorted in the order that our system processed them (instead of the default order, submitted_at).
   * This ensures that you can traverse the complete set of responses without repeating entries.
   * @param args.completed true if form was submitted. Otherwise, false.
   * @param args.sort Order of responses. Currently, responses are automatically sorted by submitted_at,desc---the date they were submitted,
   ...
   * @returns Promise that resolves with a representation of collection of responses to a specific form.
